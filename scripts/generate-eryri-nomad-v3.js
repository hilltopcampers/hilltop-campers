...
async function generateSVGDecalsV3() {
  ...
  console.log('\nGenerating Eryri NOMAD decals V3 (with "By Hilltop Campers")...\n');
  ...
  // Text positioning - adjusted for shorter "NOMAD" text
  const textGap = 15;
  const nomadFontSize = 64;
  ...
  const nomadY = 130;
  const taglineY = 158;
  const totalWidth = 850; // Slightly narrower since NOMAD is shorter than ADVENTURER
  const logoY = Math.floor((height - logoHeight) / 2);

  // Calculate right-aligned position for tagline (under NOMAD)
  const nomadWidth = nomadFontSize * 4.2; // NOMAD is 5 chars, approx width
  const taglineX = textX + nomadWidth;
...
<svg width="${totalWidth}" height="${height}" viewBox="0 0 ${totalWidth} ${height}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  <!-- Eryri NOMAD Decal V3 - ${config.name} -->
  <!-- ${config.description} -->
  ...
  <!-- NOMAD text (larger, below) -->
  <text x="${textX}" y="${nomadY}"
        font-family="Arial Black, Helvetica Neue, Arial, sans-serif"
        font-size="${nomadFontSize}"
        font-weight="900"
        fill="${config.color}"
        letter-spacing="1">NOMAD</text>

  <!-- By Hilltop Campers tagline (right-aligned under NOMAD) -->
  <text x="${taglineX}" y="${taglineY}"
    ...
    // Save SVG file
    const svgFilename = `eryri-nomad-decal-${config.name}-v3.svg`;
  ...
  for (const config of decalConfigs) {
    const svgPath = path.join(outputDir, `eryri-nomad-decal-${config.name}-v3.svg`);
    const svgContent = fs.readFileSync(svgPath);

    // Generate PNG at 2x resolution
    const pngFilename = `eryri-nomad-decal-${config.name}-v3-large.png`;
    ...
    const printFilename = `eryri-nomad-decal-${config.name}-v3-print-highres.png`;
  ...
  console.log(`\nAll Eryri NOMAD V3 decals saved to: ${outputDir}`);
  console.log('\nV3 NOMAD files generated (with "By Hilltop Campers"):');

  const files = fs.readdirSync(outputDir).filter(f => f.includes('nomad') && f.includes('-v3'));
  files.forEach(file => {
