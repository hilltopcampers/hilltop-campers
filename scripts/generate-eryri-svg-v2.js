...
async function generateSVGDecalsV2() {
  ...
  console.log('\nGenerating SVG decals V2 (with "By Hilltop Campers")...\n');
  ...
  // Text positioning
  const textGap = 15;
  const adventurerFontSize = 64;
  const eryriFontSize = 32;
  const taglineFontSize = 16;
  const textX = logoWidth + textGap;
  const eryriY = 70;
  const adventurerY = 130;
  const taglineY = 158;
  const totalWidth = 1020;
  const logoY = Math.floor((height - logoHeight) / 2);

  // Calculate right-aligned position for tagline
  const adventurerWidth = adventurerFontSize * 6.2;
  const taglineX = textX + adventurerWidth;
...
<svg width="${totalWidth}" height="${height}" viewBox="0 0 ${totalWidth} ${height}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  <!-- Eryri Adventurer Decal V2 - ${config.name} -->
  <!-- ${config.description} -->
  ...
  <!-- By Hilltop Campers tagline (right-aligned under ADVENTURER) -->
  <text x="${taglineX}" y="${taglineY}"
    ...
    // Save SVG file
    const svgFilename = `eryri-adventurer-decal-${config.name}-v2.svg`;
  ...
  for (const config of decalConfigs) {
    const svgPath = path.join(outputDir, `eryri-adventurer-decal-${config.name}-v2.svg`);
    const svgContent = fs.readFileSync(svgPath);

    // Generate PNG at 2x resolution
    const pngFilename = `eryri-adventurer-decal-${config.name}-v2-large.png`;
    ...
    const printFilename = `eryri-adventurer-decal-${config.name}-v2-print-highres.png`;
  ...
  console.log(`\nAll V2 decals saved to: ${outputDir}`);
  console.log('\nV2 files generated (with "By Hilltop Campers"):');

  const files = fs.readdirSync(outputDir).filter(f => f.includes('-v2'));
  files.forEach(file => {
...
generateSVGDecalsV2().catch(console.error);
